...
test_preflight_contract() { assert_file_contains "$WORKFLOW" 'GH_TOKEN: ${{ github.token }}' && assert_file_contains "$WORKFLOW" 'GH_TOKEN: ${{ secrets.WORKFLOW_TOKEN }}' && assert_file_contains "$WORKFLOW" 'WORKFLOW_TOKEN: ${{ secrets.WORKFLOW_TOKEN }}' && assert_file_contains "$WORKFLOW" 'GIT_CONFIG_VALUE_0="AUTHORIZATION: basic $auth_header"' && ! grep -Fq -- 'WORKFLOW_CHANGED:' "$WORKFLOW" && ! grep -Fq -- 'GITHUB_TOKEN: ${{ github.token }}' "$WORKFLOW"; }
