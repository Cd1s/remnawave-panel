...
test_workflow_contract() {
    assert_file_contains "$WORKFLOW" 'schedule:' || return 1; assert_file_contains "$WORKFLOW" 'workflow_dispatch:' || return 1; assert_file_contains "$WORKFLOW" 'UPSTREAM_SYNC_REPORT_PATH' || return 1; assert_file_contains "$WORKFLOW" 'actions/upload-artifact@v4' || return 1; assert_file_contains "$WORKFLOW" 'if: ${{ always() && steps.sync.outcome == '\''failure'\'' }}' || return 1; assert_file_contains "$WORKFLOW" 'git push origin HEAD:singbox' || return 1; assert_file_contains "$WORKFLOW" 'Verify pushed final commit' || return 1; assert_file_contains "$WORKFLOW" 'Sync official fork Release' || return 1; assert_file_contains "$WORKFLOW" 'if: steps.sync.outcome == '\''success'\''' || return 1; assert_file_contains "$WORKFLOW" 'official_panel_release_tag=' || return 1; if grep -Eq '(__RW_METADATA_VERSION|RWNODE_VERSION)=[0-9]' "$WORKFLOW"; then return 1; fi
    local merge_line push_line verify_line release_line; merge_line="$(grep -n -m1 'git merge --no-edit' "$WORKFLOW" | cut -d: -f1)"; push_line="$(grep -n -m1 'git push origin HEAD:singbox' "$WORKFLOW" | cut -d: -f1)"; verify_line="$(grep -n -m1 'name: Verify pushed final commit' "$WORKFLOW" | cut -d: -f1)"; release_line="$(grep -n -m1 'name: Sync official fork Release' "$WORKFLOW" | cut -d: -f1)"; [ -n "$merge_line" ] && [ "$merge_line" -lt "$push_line" ] && [ "$push_line" -lt "$verify_line" ] && [ "$verify_line" -lt "$release_line" ]
}
run_case() { if "$1"; then pass "$1"; else fail "$1"; fi; }
run_case test_resolve; run_case test_historical_skip; run_case test_create; run_case test_different_tag_fails; run_case test_missing_tag_fails; run_case test_query_fails; run_case test_structure_fails; run_case test_workflow_contract
