...
test_preflight_contract() { assert_file_contains "$WORKFLOW" 'GH_TOKEN: ${{ github.token }}' && assert_file_contains "$WORKFLOW" 'GH_TOKEN: ${{ secrets.WORKFLOW_TOKEN || secrets.GITHUB_TOKEN }}' && assert_file_contains "$WORKFLOW" 'GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}' && assert_file_contains "$WORKFLOW" 'WORKFLOW_TOKEN: ${{ secrets.WORKFLOW_TOKEN }}' && assert_file_contains "$WORKFLOW" 'WORKFLOW_CHANGED: ${{ steps.sync.outputs.workflow_changed }}' && assert_file_contains "$WORKFLOW" 'push_token="$GITHUB_TOKEN"' && assert_file_contains "$WORKFLOW" 'push_token="$WORKFLOW_TOKEN"' && ! grep -Fq -- 'PACKAGE_TOKEN' "$WORKFLOW" && ! grep -Fq -- 'permissions.push' "$WORKFLOW" && ! grep -Fq -- 'actions/workflows' "$ROOT/.github/scripts/upstream-sync-lib.sh" && ! grep -Fq -- 'SKIP_GIT_DRY_RUN' "$ROOT/.github/scripts/upstream-sync-lib.sh"; }
